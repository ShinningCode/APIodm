...
describe('Prueba de inicio de sesión', () => {
  it('debería iniciar sesión y obtener una clave de acceso', async () => {
    const response = await supertest(app)
      .post('/login')
      .send({ username: 'usuario', password: 'contraseña' })
      .expect(200); // Espera un código de estado HTTP 200

    // Guarda la clave de acceso para usarla en las pruebas posteriores
    key = response.body.token;
  });
});

// Prueba de la función crear un actor
describe('Prueba de creación de un nuevo actor', () => {
...
describe('DELETE /api/actors/:id', () => {
  it('debería eliminar un actor específico', async () => {
    ...
    await supertest(app).get(/api/actors/`${id}`)
        .set('Authorization', Bearer `${key}`)
